...
export async function POST() {
  try {
    ...
    const project1 = await prisma.project.create({
      data: {
        name: '고객 포털 리뉴얼',
        description: '고객 포털 UI/UX 전면 개편 프로젝트',
        color: '#6366f1',
        status: 'ACTIVE',
    ...
    const project2 = await prisma.project.create({
      data: {
        name: 'API 고도화',
        description: 'REST API 성능 개선 및 신규 엔드포인트 개발',
        color: '#22c55e',
        status: 'ACTIVE',
    ...
    await prisma.issue.create({
      data: {
        ...
        reporterId: user2.id,
        assigneeId: admin.id,
        title: '반응형 레이아웃 깨짐 이슈',
        description: '모바일 환경에서 UI가 올바르게 표시되지 않음',
        severity: 'HIGH',
        status: 'OPEN',
      },
    })
    ...
    await prisma.issue.create({
      data: {
        ...
        reporterId: user1.id,
        title: 'Redis 연결 타임아웃',
        description: '특정 상황에서 Redis 연결이 지연됨',
        severity: 'CRITICAL',
        status: 'IN_PROGRESS',
      },
    })
    ...
    return NextResponse.json({ message: '초기 데이터 생성 완료!' })
  } catch (error) {
